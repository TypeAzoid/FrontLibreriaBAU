...
class Clientes extends Component {
  ...
  async concreteEdit(nombre, direccion) {
    ...
    if (this.state.clienteid !== "0") {
      await ClienteService.editarCliente(
        this.state.clienteid,
        this.state.clientename,
        this.state.clientedir
      );
  ...
  async borrarCliente(e) {
    ...
    const incluidas = suscripciones.filter(filt =>
      filt.cliente.name.toLowerCase().includes(cliente.name.toLowerCase())
    );
    if (incluidas.length >= 1) {
      // eslint-disable-next-line no-restricted-globals
      const opcion = confirm(
        "El cliente tiene suscripciones, ¿desea eliminarlas?"
      );
      if (opcion === true) {
        ...
        // eslint-disable-next-line no-restricted-globals
        let opcion2 = confirm(
          "Si borra este cliente se borraran: " +
            idsIncluidas.length +
            " suscripciones ¿desea continuar?"
        );
        if (opcion2 === true) {
  ...
  async listarClientes() {
    ...
    if (this.state.buscador !== "") {
      if (this.state.busqueda === "0") {
        let data = clientes.filter(filt =>
          filt.name.toLowerCase().includes(this.state.buscador.toLowerCase())
        );
        ...
        let data = clientes.filter(
          filt => filt.id === parseInt(this.state.buscador)
        );
        await this.setState({ clientes: data });
      }
    } else {
